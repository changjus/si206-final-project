...
def get_restaurants_info(data):
    tup = []
    restaurant_list = data.get("businesses")
...
def get_yelp_data(location):
    ...
    parameters = {'term':'restaurant', 'limit':20, 'radius':9000, 'location':location}

    restaurants = requests.get(url=search, params=parameters, headers=headers)
    data = restaurants.json() 
    restaurants_info = get_restaurants_info(data)
    conn = sqlite3.connect('data.db')
    cur = conn.cursor()
    cur.execute('''CREATE TABLE IF NOT EXISTS Restaurants (name TEXT UNIQUE PRIMARY KEY, city TEXT)''')
    ...
    for info in restaurants_info:
        cur.execute('''INSERT OR IGNORE INTO Restaurants VALUES (?, ?)''', (info[0], info[3]))  
...
def main():
    ...
    if answer == '1':
        ...
        if start <= 0:
            start = 1
        get_list_cities_population(start - 1)
