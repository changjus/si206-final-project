...
def make_join():
    ...
    cur.execute('''INSERT OR IGNORE INTO Yelp_Data SELECT * FROM Metrics JOIN Restaurants USING (name)''')
...
def output_data(rating, price, cities):
    output = open("calculations.txt", "w")
    output.write("City, Average Rating\n")
    for i in range(len(rating)):
        output.write(cities[i] + ", " + str(rating[i]) + "\n")
    output.write("\n")
    output.write("City, Average Price\n")
    for i in range(len(price)):
        output.write(cities[i] + ", " + str(price[i]) + "\n")
    output.close()
...
def main():
    ...
    output_data(avg_rating_list, avg_price_list, cities_list)
    return 0
